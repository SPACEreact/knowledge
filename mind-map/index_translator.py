...
for tag in soup.find_all(['h1','h2','h3','h4','p','li','a','span','div']):
    # Only translate elements with actual text and no block children
    if tag.has_attr('data-i18n') or tag.has_attr('data-i18n-html'): continue
    text = tag.get_text(strip=True)
    if len(text) < 3: continue
    
    # Check if purely inline
    valid = True
...
print("Translated", count, "elements on index.html")
